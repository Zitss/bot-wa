let handler = async (m, { conn, text }) => {
  conn.kontolbapak = conn.kontolbapak ? conn.kontolbapak : {}
  if (m.chat in conn.kontolbapak) throw 'Masih ada yang sedang membuat\nDeus\ndi chat ini... tunggu sampai selesai'
  else conn.kontolbapak[m.chat] = true
  m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'au ah pusing', m)
  } finally {
    delete conn.kontolbapak[m.chat]
  }
}
handler.help = ['ktl <teks>']
handler.tags = ['tools']
handler.command = /^((ktl)?ktl)$/i
handler.limit = true
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let font = path.join(_font, 'Futura Bold font.ttf')
    let w = 1024
    let h = w
    let s = w + 'x' + h
    ...
    let layers = [
      ...
      textArgs('KONTOL', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('BAPAK', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
    ...
    let o = 1 * new Date + '_kontol_bapak.png'
    o = path.join(tmp, o)
    let args = [
      '-y',
      '-i', img,
